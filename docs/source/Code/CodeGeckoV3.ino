...
int customCourseMapped; // Pour stocker la consigne de la course
...
void setup() {                
 ...
 /* while (!(course_max + 150 == steps_init)) { // Tant que la toute la course + un chouillla n'est pas accomplie, on va vers la butée
  ...
  if(CoteMilieu){
    while(steps<(course_max/2)){  // va jusqu'à au milieu
      digitalWrite(step_pin, HIGH);
      delay(1);          
      digitalWrite(step_pin, LOW); 
      delay(1);
...
void loop() {
  ...
  while ((customSpeedMapped) > (v_min - 17)){ // si le potentiomètre de la vitesse est au minimum, on met LM42 à l'arrêt
 // int a=1;
  //customSpeedMapped =speedUp(); // lecture de la vitesse
